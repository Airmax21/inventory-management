<template>
    <a-modal ref="modalRef" v-model:open="modalShow" centered :keyboard="false" title="Create Masters" ok-text="Save"
        cancel-text="Cancel" :confirm-loading="isPending" :mask-closable="!isPending" :closable="false" destroy-on-close
        @ok="modalHandleOk">
        <master-form ref="formRef" :disabled="isPending" />
    </a-modal>
</template>
...
const { isPending, mutate } = useMutation({
    mutationFn: api.master.create,
    onSuccess: () => {
        modalHandleClose();
        if (formRef.value) {
            formRef.value.reset();
        }
        message.success('Create Master Successful')
