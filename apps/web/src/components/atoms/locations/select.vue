...
const value = defineModel<ILocation['id']>('value');
const search = ref<string>();
const searchDebounced = refDebounced(search, 500);
const queryClient = useQueryClient();

watch(searchDebounced, () => {
    queryClient.setQueryData<InfiniteData<ApiPaginateResponse<ILocation>, number>>(
        ['locations/select'],
        (data) => !data ? undefined : {
...
const { data, fetchNextPage, hasNextPage, isFetching } = useInfiniteQuery({
    queryKey: ['locations/select', searchDebounced],
    queryFn: async ({ pageParam = 1 }) => {
        const { data } = await api.location.paginate({
            page: pageParam,
