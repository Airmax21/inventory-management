...
const value = defineModel<ICategory['id']>('value');
const search = ref<string>();
const searchDebounced = refDebounced(search, 500);
const queryClient = useQueryClient();

watch(searchDebounced, () => {
    queryClient.setQueryData<InfiniteData<ApiPaginateResponse<ICategory>, number>>(
        ['categories/select'],
        (data) => !data ? undefined : {
...
const { data, fetchNextPage, hasNextPage, isFetching } = useInfiniteQuery({
    queryKey: ['categories/select', searchDebounced],
    queryFn: async ({ pageParam = 1 }) => {
        const { data } = await api.category.paginate({
            page: pageParam,
